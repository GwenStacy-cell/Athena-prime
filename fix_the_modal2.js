...
const regexModal = /if \(interaction\.customId\.startsWith\('gw_manage_modal_'\)\) \{[\s\S]*?return interaction\.reply\(\{ embeds: \[embed\], components: \[row\], ephemeral: true \}\);\n\s*\}/g;
code = code.replace(regexModal, "");

const regexButton = /if \(interaction\.customId\.startsWith\('gw_manage_'\)\) \{[\s\S]*?return interaction\.showModal\(modal\);\n\s*\}/g;
...
code = code.replace(regexButton, newDropdownCode);
