import fs from "fs";

let js = fs.readFileSync("src/commands/loader.js", "utf8");

// Add import
const importStr = "import { commands as botstatsCmds } from './botstats.js';";
const newImportStr = "import { commands as botstatsCmds } from './botstats.js';\nimport uploadCmd from './upload.js';";
js = js.replace(importStr, newImportStr);

// Add to array
const arrayStr = "...botstatsCmds";
const newArrayStr = "...botstatsCmds,\n  uploadCmd";
js = js.replace(arrayStr, newArrayStr);

fs.writeFileSync("src/commands/loader.js", js);
