...
if (match) {
    ...
    for (const [id, cmds] of Object.entries(newCommands)) {
        // Matches { id: 'security', ... ] },
        const modRegex = new RegExp(`\\{\\s*id:\\s*'${id}'.*?\\]\\s*\\},?`, 'g');
        
        rawArray = rawArray.replace(modRegex, (fullMatch) => {
            const prefix = fullMatch.substring(0, fullMatch.indexOf('commands: '));
            const hasComma = fullMatch.endsWith(',');
            return prefix + `commands: ` + JSON.stringify(cmds) + ` }` + (hasComma ? ',' : '');
        });
    }
    
    js = js.replace(regex, rawArray);
    fs.writeFileSync("src/commands/utility.js", js);
    console.log("Injected all missing commands cleanly!");
} else {
