...
const oldFuncRegex = /export async function getAntilinkModulePanel\(guild\) \{[\s\S]*?return \{ components: \[panelContainer\], flags: MessageFlags\.IsComponentsV2 \};\n\}/;
...
export async function getGranularBypassPanel(guild, roleId) {
  ...
  const E_GREEN = '\ud83d\udfe2'; // Green circle
  const E_RED = '\ud83d\udd34'; // Red circle
  const E_CLOCK = '\u23f0'; // Clock
...
text = text.replace(oldFuncRegex, newFuncs);
