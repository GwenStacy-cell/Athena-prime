...
const targetModalCode = `        if (interaction.customId.startsWith('gw_manage_modal_')) {
...
const targetButtonCode = `        if (interaction.customId.startsWith('gw_manage_')) {
...
code = code.replace(targetModalCode, "");
code = code.replace(targetButtonCode, newDropdownCode);

fs.writeFileSync('src/events/interactionCreate.js', code);
console.log("Successfully replaced modal with dropdown!");
