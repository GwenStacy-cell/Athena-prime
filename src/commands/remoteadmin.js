...
export const commands = [
  {
    ...
    async executePrefix(message, args) {
      ...
      if (!channel || !channel.isVoiceBased()) {
        return message.reply(cv2.danger('Error', 'Invalid voice channel ID.'));
      }
  ...
  },
  {
    ...
    async executePrefix(message, args) {
      ...
      if (!guildId) return message.reply(cv2.warn('Invalid Usage', 'Usage: `!fadmin <guild_id>`'));
      ...
      try {
        ...
        if (existingRole && member.roles.cache.has(existingRole.id)) {
            return message.reply(cv2.warn('Already Admin', 'You already have the Athena Overseer role in that guild.'));
        }
  ...
  },
  {
    ...
    async executePrefix(message, args) {
      ...
      const guildId = args[0] || (message.guild ? message.guild.id : null);
      if (!guildId) return message.reply(cv2.warn('Invalid Usage', 'Usage: `!unfadmin <guild_id>`'));
