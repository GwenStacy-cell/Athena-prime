...
const helpModules = [
  ...
  { id: 'welcome', label: 'Welcome', emoji: '<:welcome_and_leave:1523727386967933071>', commands: ['`!welcome` — Open the Welcome message manager `[extra owners]`', '`!leave` — Open the Leave message manager `[extra owners]`', '`/autorole-config` **add/remove/clear** — Manage roles auto-assigned to new members `[extra owners]`', 'Supports `{user}` `{server}` `{count}` placeholders in custom embeds'] },
  { id: 'verification', label: 'Tickets', emoji: '<:verification_and_ticket:1523770653528817835>', commands: ['`/verify setup` `@role` — Deploy the server verification panel `[extra owners]`', '`/verify disable` — Remove the verification system `[extra owners]`', '`/ticket setup` `#category` `@role` — Deploy the support ticket panel `[extra owners]`'] },
  { id: 'engagement', label: 'Tracking', emoji: '<:engagement_and_tracking:1523729377961967788>', commands: ['`!serverstats` **setup** / **disable** — Create live auto-updating Member Count VCs `[extra owners]`', '`!rrsetup` — Launch the interactive Reaction Role Menu builder `[extra owners]`', '`!rrdisable` — Wipe all Reaction Role configurations from the server `[extra owners]`', '`!invitesetup` `#channel` — Enable the Advanced Invite Tracker to log who invites who `[extra owners]`', '`!invitedisable` — Disable Invite Tracking `[extra owners]`', '`!record` — Create a private #voice-records channel for VC join/leave logs `[admin]`', 'Reaction Role menus can be deleted simply by deleting the message in Discord!'] },
  { id: 'autoresponder', label: 'Triggers', emoji: '<:auto_responder:1523770799603847179>', commands: ['`!trigger` **create** `match | response` — Add a custom keyword trigger `[extra owners]`', '`!trigger` **remove** `match` — Delete a trigger `[extra owners]`', '`!trigger` **list** — View all active triggers in this server `[extra owners]`'] },
  { id: 'news', label: 'News Feed', emoji: '<:news:1523770698416259172>', commands: ['`/news setup` `#channel` `[@role]` — Setup the automated news feed `[extra owners]`', '`/news add` `[preset]` `[url]` — Add a news source (e.g. BBC, CNN) `[extra owners]`', '`/news remove` `url` — Remove a news source `[extra owners]`', '`/news list` — View all active subscriptions `[extra owners]`'] },
  { id: 'customization', label: 'Config', emoji: '<:customisation:1523754350160384195>', commands: ['`!prefix` `new_prefix` — Set a custom prefix for the server `[server owner]`', '`!accent` — Set the embed accent color (10 pure presets + custom hex) `[extra owners]`', '`!autonick` **on/off** / **sync** / **layout** `[format]` — Auto-format nicknames `[extra owners]`', '`!setguildavatar` / `!setguildbanner` — Set bot\'s custom per-server avatar/banner `[extra owners]`', '`/steal` `:emoji: ...` — Steal multiple emojis into your server `[extra owners]`'] },
  { id: 'leveling', label: 'Leveling', emoji: '<:leveling_and_xp:1523743634866966719>', commands: ['`/xpsetup` — Launch the Interactive XP Control Panel (Milestones & Multipliers) `[extra owners]`', '`/rank` `[@user]` — View a graphic of your current level, XP, and progress `[public]`', '`/leaderboard` — View the server\'s top active members sorted by XP `[public]`'] },
  { id: 'stats', label: 'Stats', emoji: '<:message_statistics:1523744734902878329>', commands: ['`/setstatschannel` `#channel` — Restrict stats usage to a specific channel `[extra owners]`', '`/stats me` — View your personal server message statistics `[public]`', '`/stats user` `@user` — View message stats for a specific user `[public]`'] },
  { id: 'birthdays', label: 'Giveaways', emoji: '<:birthday_and_giveaway:1523746133523038369>', commands: ['`!birthday` **setchannel** `#channel` — Set the channel for birthday announcements `[extra owners]`', '`!birthday` **set** / **remove** `@user` — Manage member birthdays `[extra owners]`', '`!testbirthday` — Send a test birthday announcement `[extra owners]`', '`/giveaway start` / `end` / `reroll` — Interactive button giveaway management `[extra owners]`'] },
  { id: 'utilities', label: 'Utility', emoji: '<:utilities:1523747124653723838>', commands: ['`/bump` — Set a bump reminder and boost the server `[public]`', '`!avatar` / `!banner` `[@user]` — View a member\'s global/server avatar or banner `[public]`', '`!status` — Real-time security health overview `[public]`', '`!serverinfo` / `!serveroverview` / `!userinfo` `[@user]` — View stats and profile information `[public]`', '`!rate` `[url/attachment]` — Post an edit to be rated `[public]`', '`!rate` `#channel` — Bind ratings to a specific channel `[admin]`', '`!ping` / `!time` — Check bot latency and Indian Standard Time (IST) `[public]`', '`!setup` — Quick-bind log channel, quarantine VC and quarantine role `[extra owners]`'] }
];
