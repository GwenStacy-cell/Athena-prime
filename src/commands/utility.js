...
const helpModules = [
  ...
  { id: 'voice', shortLabel: 'Voice', label: 'Voice & Join-to-Create', emoji: '<:voice_join_to_create:1523770607706308658>', commands: ['**Voice Channel Moderation**', '`!theatermode` **on/off** — Activates Movie Mode (Server mutes/deafens the entire VC) `[extra owners]`', '`!vclock` / `!vcunlock` — Deny or restore Connect permissions for @everyone in your VC `[extra owners]`', '`!mute` / `!unmute` / `!deafen` / `!undeafen` — VC member state control `[extra owners]`', '`!muteall` / `!unmuteall` / `!deafenall` / `!undeafenall` — Mass VC state control `[extra owners]`', '`!vcstatus` **on/off** — Toggle the dynamic VC live status text `[extra owners]`', '', '**Protections & Security**', '`!moveprotect` **add/remove/list** `@user` — Prevent admins from moving protected users `[server owner]`', '`!vcprotect` **add/remove/list** `@user` — Prevent admins from muting/deafening protected users `[server owner]`', '', '**Mass Voice Control**', '`!massmove` `dest` / `!massdc` — Move or disconnect everyone in a VC `[extra owners]`', '`!vcdrag` `@user` `[interval]` — Drag a user endlessly through VCs (default: 2s) `[extra owners]`', '`!vcdragstop` `@user` — Stop the drag session for a specific user `[extra owners]`', '`!vcdraglist` — View all currently active drag sessions `[extra owners]`', '', '**Join-to-Create (JTC)**', '`!jtcsetup` `#voicechannel` — Designate the JTC creator channel `[extra owners]`', '`!jtcdisable` — Remove the JTC system from this server `[extra owners]`', '`!vc` — Manage your personal JTC channel (rename, limit, privacy...) `[public]`', '', '**Bot Configuration**', '`!sethomevc` `[channel]` — Set bot\'s Home VC (auto-rejoin if moved) `[extra owners]`', '`!unsethomevc` — Clear Home VC and disconnect the bot `[extra owners]`'] },
