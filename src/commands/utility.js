...
const helpModules = [
  { id: 'security', shortLabel: 'Security', label: 'Security & Firewall', emoji: '<:security_and_firewall:1523672289500069940>', commands: ['`!security` **enable all** / **disable all** — Toggle all shields `[extra owners]`', '`!scanserver` — Scan and manage unauthorized bots `[extra owners]`', '`!lockapps` / `!unlockapps` — Manage slash commands server-wide `[extra owners]`', '`!antinuke` **config** — Open the interactive configuration panel `[extra owners]`', '`!config` **antinuke** / **antispam** / **antiinvite** / **antibot** / **maxwarnings** `on|off` `[extra owners]`', '`!moveprotect` **add/remove/list** `@user` — Prevent admins from moving protected users `[server owner]`', '`!vcprotect` **add/remove/list** `@user` — Prevent admins from muting/deafening protected users `[server owner]`', '`!raidmode` **on** / **off** — Auto-quarantine every new join during a raid `[extra owners]`', '`!emergency` **mode** / **end** — Strip dangerous permissions and hide channels `[extra owners]`', 'You MUST whitelist friendly bots (`!botwhitelist add <ID>`). Unwhitelisted bots will be instantly banned.'] },
  ...
  { id: 'vcdrag', shortLabel: 'VC Drag', label: 'VC Drag', emoji: '<:vc_drag:1523723288663298291>', commands: ['`!vcdrag` `@user` `[interval]` — Drag a user endlessly through VCs (default: 2s) `[extra owners]`', '`!vcdragstop` `@user` — Stop the drag session for a specific user `[extra owners]`', '`!vcdraglist` — View all currently active drag sessions `[extra owners]`'] },
  ...
  { id: 'verification', shortLabel: 'Tickets', label: 'Verification & Tickets', emoji: '<:verification_and_ticket:1523770653528817835>', commands: ['**Verification**', '`/verify setup` `@role` — Deploy the server verification panel `[extra owners]`', '`/verify disable` — Remove the verification system `[extra owners]`', '', '**Default Ticket System**', '`!ticket setup` `#category` `@role` — Deploy a simple, single-button ticket system `[extra owners]`', '', '**Custom Ticket Panel**', '`!ticketpanel` — Spawns the Interactive Ticket Manager with the following buttons:', '• **Edit Title & Desc**: Changes the main text of the Ticket Panel.', '• **Edit Media & Placeholder**: Attach images and change the dropdown placeholder.', '• **Add Option**: Adds a new selectable reason to the dropdown menu.', '  - **Internal Value**: The secret code word for the bot (e.g. `billing_support`). Must be unique and have no spaces.', '  - **Display Label**: The bold text the user actually clicks on.', '  - **Description**: The smaller gray text under the label.', '  - **Emoji**: An optional Emoji ID (e.g. `1517802018`) or standard emoji.', '• **Clear Options**: Instantly deletes ALL dropdown options.', '• **Deploy Panel**: Drops the final customized Ticket Panel into the chat.', '• **Close**: Deletes the Interactive Manager message.'] },
