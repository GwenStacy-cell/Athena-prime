...
const helpModules = [
  ...
  { id: 'whitelist', shortLabel: 'Whitelist', label: 'Whitelist & Permissions', emoji: '<:whitelist_and_permissions:1523678393269223564>', commands: ['`!whitelist` **add** / **remove** / **list** `@user|botId` `[events...]` `[extra owners]`', 'Events: `all` `antinuke` `antibot` `antispam` `antilink` `antiinvite` `quarantine`', '`!botwhitelist` **add** / **remove** / **list** `botId|@role` — Grant full Anti-Nuke immunity to bots/roles `[server owner]`', '`!userblacklist` **add** / **remove** / **list** `@user` — Blacklist a user from the bot `[extra owners]`', '`!extraowner` **add** / **remove** / **list** `@user` — Grant full bot access `[server owner]`'] },
  ...
  { id: 'moderation', shortLabel: 'Moderation', label: 'Moderation & Threads', emoji: '<:moderation_and_threads:1523770550638346380>', commands: ['`!warn` `@user` `reason` — Issue a warning (auto-quarantine at threshold) `[extra owners]`', '`!warnings` / `!clearwarns` `@user` — View or wipe warning history `[extra owners]`', '`/maxwarnings` `amount` — Set the maximum warning threshold `[extra owners]`', '`!timeout` `@user` `dur` — Timeout a member (e.g. `5m` `1h` `1d`) `[extra owners]`', '`!kick` / `!ban` / `!unban` / `!unbanall` — Standard moderation actions `[extra owners]`', '`!addrole` / `!removerole` `@user` `@roles...` — Safely assign/remove multiple roles `[extra owners]`', '`!striproles` `@user` — Instantly strip all roles from a member `[extra owners]`', '`!sync` / `!syncall` — Sync channel permissions with category `[extra owners]`', '`!purge` `1-100` — Bulk-delete messages from current channel `[extra owners]`', '`!slowmode` `seconds` — Set channel slowmode (0 = off) `[extra owners]`', '`!createrole` / `!deleterole` — Create or delete a role `[extra owners]`', '`!hide` / `!unhide` `[channel]` — Instantly hide or unhide a text/voice channel from @everyone `[extra owners]`', '`!createthread` / `!archivethread` / `!deletethread` — Thread management `[extra owners]`'] },
  ...
  { id: 'voice', shortLabel: 'Voice', label: 'Voice & Join-to-Create', emoji: '<:voice_join_to_create:1523770607706308658>', commands: ['`!theatermode` **on/off** — Activates Movie Mode (Server mutes/deafens the entire VC) `[extra owners]`', '`!vclock` / `!vcunlock` — Deny or restore Connect permissions for @everyone in your VC `[extra owners]`', '`!mute` / `!unmute` / `!deafen` / `!undeafen` — VC member state control `[extra owners]`', '`!muteall` / `!unmuteall` / `!deafenall` / `!undeafenall` — Mass VC state control `[extra owners]`', '`!vcstatus` **on/off** — Toggle the dynamic VC live status text `[extra owners]`', '`!moveprotect` **add/remove/list** `@user` — Prevent admins from moving protected users `[server owner]`', '`!vcprotect` **add/remove/list** `@user` — Prevent admins from muting/deafening protected users `[server owner]`', '`!jtcsetup` `#voicechannel` — Designate the JTC creator channel `[extra owners]`', '`!jtcdisable` — Remove the JTC system from this server `[extra owners]`', '`!vc` — Manage your personal JTC channel (rename, limit, privacy...) `[public]`', '`!sethomevc` `[channel]` — Set bot\'s Home VC (auto-rejoin if moved) `[extra owners]`', '`!unsethomevc` — Clear Home VC and disconnect the bot `[extra owners]`'] },
  { id: 'vcdrag', shortLabel: 'Mass VC', label: 'Mass Voice Control', emoji: '<:vc_drag:1523723288663298291>', commands: ['`!massmove` `dest` / `!massdc` — Move or disconnect everyone in a VC `[extra owners]`', '`!vcdrag` `@user` `[interval]` — Drag a user endlessly through VCs (default: 2s) `[extra owners]`', '`!vcdragstop` `@user` — Stop the drag session for a specific user `[extra owners]`', '`!vcdraglist` — View all currently active drag sessions `[extra owners]`'] },
  ...
  { id: 'engagement', shortLabel: 'Tracking', label: 'Engagement & Tracking', emoji: '<:engagement_and_tracking:1523729377961967788>', commands: ['`!serverstats` **setup** / **disable** / **config** — Create & configure live Member Count VCs `[extra owners]`', '`!rrsetup` — Launch the interactive Reaction Role Menu builder `[extra owners]`', '`!rrdisable` — Wipe all Reaction Role configurations from the server `[extra owners]`', '`!invitesetup` `#channel` — Enable the Advanced Invite Tracker to log who invites who `[extra owners]`', '`!invitedisable` — Disable Invite Tracking `[extra owners]`', '`!record` — Create a private #voice-records channel for VC join/leave logs `[extra owners]`', 'Reaction Role menus can be deleted simply by deleting the message in Discord!'] },
