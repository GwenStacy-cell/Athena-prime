...
const oldTOS = `export async function getServerSecurityEnabledPanel() {
...
const newTOS = `export async function getServerSecurityEnabledPanel() {
    ...
    const section2 = {
        ...
        content: "<a:warning:1540656124313993247> **\`!botwhitelist add <bot id>\` - To add bots to server by whitelisted users before that the admin bot comes in either whitelisted to ensure data leaving server owner or extra owner**\\n\\n" +
                 ...
                 "<a:warning:1540656124313993247> **\`!security dashboard\` - A switch dashboard shows server security status and you can configure action not for action tags in as command**\\n\\n" +
                 "<a:warning:1540656124313993247> **\`#security-logs\` - athena prime a webhook created for logging artifacts**"
    };
...
// Since string matching multiline can be tricky, let's use indexOf slice logic
const startIdx = code.indexOf("export async function getServerSecurityEnabledPanel");
const endIdx = code.indexOf("async function getSecureDashboardPanel");
...
if (startIdx !== -1 && endIdx !== -1) {
    ...
    console.log("Could not find TOS panel block!");
}
