...
let code = fs.readFileSync("original_security.js", "utf8");
...
if (parts.length === 3) {
    const prefixInject = `
      ...
      const p1Tail = parts[1].substring(parts[1].indexOf("await message.reply(result);") + "await message.reply(result);".length);
      const p2Tail = parts[2].substring(parts[2].indexOf("await interaction.reply(result);") + "await interaction.reply(result);".length);
      
      let finalCode = parts[0] + "} else if (disable) {" + prefixInject + p1Tail + "} else if (disable) {" + slashInject + p2Tail;
