...
const regex1 = /const modLabels = \{[^}]*?antiInvite: 'Anti Invite \(Danger\)'[^}]*?\};/;
const regex2 = /const modLabels = \{[^}]*?antiAppCommands: 'Anti App Commands'[^}]*?\};/;

sec = sec.replace(regex1, replacement);
sec = sec.replace(regex2, replacement);

fs.writeFileSync("src/commands/security.js", sec);
console.log("Replaced:", sec.includes("[0ms WSS]"));
