...
// Remove the old blocks correctly using global regex
...
// Inject at the top
const regexInject = /async execute\(interaction\) \{[\s\S]*?if \(!interaction\.guild[\s\S]*?\) return;/;
const replacement = `async execute(interaction) {
...
js = js.replace(regexInject, replacement);
