...
if (js.includes('// Ignore bots and webhooks')) {
    js = js.replace(/[\t ]*\/\/ Ignore bots and webhooks[\r\n]+[\t ]*if \(message\.author\.bot \|\| message\.webhookId\) return;/, newStr);
    fs.writeFileSync("src/events/messageCreate.js", js);
    console.log("Successfully injected and logged bot spam filter.");
} else {
    console.log("Target string not found in messageCreate.js");
}
