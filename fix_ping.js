...
const prefixOld = `      async executePrefix(message) {
...
const prefixNew = `      async executePrefix(message) {
        ...
        await sent.edit({ components: [{ type: 17, components: comps }], files: [attachment] });
      }`;

const slashOld = `      async executeSlash(interaction) {
        const { EmbedBuilder, AttachmentBuilder } = await import('discord.js');
        const { generatePingGraph } = await import('../utils/graph.js');
...
const slashNew = `      async executeSlash(interaction) {
        const { AttachmentBuilder, MessageFlags } = await import('discord.js');
        const { generatePingGraph } = await import('../utils/graph.js');
        const cfg = db.getGuildConfig(interaction.guild?.id || '0');
        const accentHex = cfg?.accentColor || '#00e5ff';
        ...
        await interaction.editReply({ components: [{ type: 17, components: comps }], files: [attachment] });
      }`;

code = code.replace(prefixOld, prefixNew);
code = code.replace(slashOld, slashNew);
