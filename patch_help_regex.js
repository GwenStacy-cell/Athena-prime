...
const regex = /async executePrefix\(message\) \{\s+let reply;\s+try \{\s+const components = buildHelpContainer\(message\.client, message\.guild\?\.id, 'home'\);\s+reply = await message\.reply\(\{ components: \[components\], flags: MessageFlags\.IsComponentsV2 \}\);\s+\} catch \(e\) \{\s+return message\.channel\.send\(\{ content: `\*\*DEBUG ERROR:\*\* \\\`\$\{e\.message\}\\\`` \}\)\.catch\(\(\) => null\);\s+\}/;
...
const newPrefix = `async executePrefix(message, args) {
        ...
        try {
          if (args && args.length > 0) {
              ...
              if (!targetCmd) {
                  return message.reply({ content: \`-! \u26A0\uFE0F **Command Not Found:** \` + cmdName, flags: MessageFlags.Ephemeral });
              }
