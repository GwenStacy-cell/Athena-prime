...
const helpIndex = js.indexOf("name: 'help',");
if (helpIndex === -1) {
    console.log("Could not find name: 'help'");
    process.exit(1);
}

const execIndex = js.indexOf("async executePrefix(message) {", helpIndex);
const catchIndex = js.indexOf("const collector = reply.createMessageComponentCollector", execIndex);
...
js = js.substring(0, execIndex) + newPrefix + js.substring(catchIndex);
fs.writeFileSync("src/commands/utility.js", js);
console.log("Help fix applied safely!");
